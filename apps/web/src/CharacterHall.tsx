...
export default function CharacterHall({ onBack, onEnterCourt }: CharacterHallProps) {
  ...
  return <main className="character-hall">
    <header className="character-hall__topbar"><button type="button" className="character-hall__back" onClick={onBack} aria-label="返回平台空间"><span className="character-hall__brand-mark" /> <b>BalaBala</b></button><nav className="character-hall__nav" aria-label="平台模块导航"><button type="button" className="is-active">角色档案</button><button type="button" onClick={onBack}>场景</button><button type="button" disabled>广场</button></nav><button type="button" className="character-hall__court" onClick={() => onEnterCourt(selected ?? undefined)}><Gavel size={15} aria-hidden="true" /> 进入趣味法庭 {selected ? `· ${selected.name}` : ''} <ChevronRight size={14} aria-hidden="true" /></button></header>
